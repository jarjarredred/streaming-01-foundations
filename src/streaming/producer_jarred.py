...
SALES_CSV: Final[Path] = DATA_DIR / "sales_jarred.csv"
...
def get_message_key(message: dict[str, Any]) -> str:
    try:
        return str(message["region_id"])  # Changed from region_id
...
def send_messages() -> int:
    ...
    try:
        for message in generate_messages(MESSAGE_COUNT):
            # === TECHNICAL CHANGE: REAL-TIME DATA ENRICHMENT ===
            # Calculate total order revenue dynamically from existing fields
            try:
                price = float(message.get("unit_price", 0))
                qty = int(message.get("quantity", 0))
                message["total_revenue"] = f"{price * qty:.2f}"
            except (ValueError, TypeError):
                message["total_revenue"] = "0.00"
            # ===================================================
